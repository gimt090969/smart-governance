...
>     <script src="https://cdn.jsdelivr.net/npm/xlsx/dist/xlsx.full.min.js"></script>
  ...
  <nav id="sidebar"></nav>
  <div id="content">
      <div id="top-navbar" class="top-navbar" data-title="ทะเบียนครัวเรือน"></div>
      <div class="page-content">
          ...
          <div class="d-flex align-items-center justify-content-between mb-3 flex-wrap gap-2">
              <div>
                  <h4 class="fw-bold mb-1"><i class="fa-solid fa-house-chimney text-success me-2"></i>ทะเบียนครัวเรือน</h4>
                  <nav aria-label="breadcrumb"><ol class="breadcrumb mb-0" style="font-size:0.8rem;">
                      <li class="breadcrumb-item"><a href="digital-data-dashboard.html" class="text-decoration-none">Digital Data Center</a></li>
                      <li class="breadcrumb-item active">ครัวเรือน</li>
                  </ol></nav>
              </div>
              <div class="d-flex gap-2">
                  <input type="file" id="importTR14Excel" accept=".xlsx, .xls" style="display:none;" onchange="handleImportTR14Excel(event)">
                  <div class="dropdown">
                      <button class="btn btn-sm btn-outline-primary dropdown-toggle" type="button" data-bs-toggle="dropdown" aria-expanded="false">
                          <i class="fa-solid fa-users-viewfinder me-1"></i>จัดการ ทร.14
                      </button>
                      <ul class="dropdown-menu shadow">
                          <li><a class="dropdown-item" href="#" onclick="document.getElementById('importTR14Excel').click()"><i class="fa-solid fa-file-import me-2 text-success"></i>นำเข้าข้อมูล ทร.14 (Import)</a></li>
                          <li><a class="dropdown-item" href="#" onclick="exportTR14Excel()"><i class="fa-solid fa-file-excel me-2 text-secondary"></i>ดาวน์โหลดแบบฟอร์ม (Export)</a></li>
                      </ul>
                  </div>
                  
                  <input type="file" id="importExcel" accept=".xlsx, .xls" style="display:none;" onchange="handleImportExcel(event)">
                  <button class="btn btn-sm btn-outline-success" onclick="document.getElementById('importExcel').click()"><i class="fa-solid fa-file-import me-1"></i>นำเข้าพิกัด</button>
                  <button class="btn btn-sm btn-outline-secondary" onclick="exportExcel()"><i class="fa-solid fa-file-excel me-1"></i>Export พิกัด</button>
                  <button class="btn btn-sm btn-success" onclick="showAddModal()"><i class="fa-solid fa-plus me-1"></i>เพิ่มครัวเรือน</button>
              </div>
          </div>
          ...
          <div class="card mb-3">
              <div class="card-body py-2">
                  <div class="row g-2 align-items-center">
                      <div class="col-md-5"><div class="input-group input-group-sm"><span class="input-group-text bg-light"><i class="fa-solid fa-search text-muted"></i></span><input type="text" class="form-control" id="searchInput" placeholder="ค้นหาบ้านเลขที่ / ชื่อเจ้าบ้าน / ทร.14..." onkeyup="filterTable()"></div></div>
                      <div class="col-md-3"><select class="form-select form-select-sm" id="filterVillage" onchange="filterTable()"><option value="">ทุกหมู่บ้าน</option><option value="1">หมู่ 1</option><option value="2">หมู่ 2</option><option value="3">หมู่ 3</option><option value="4">หมู่ 4</option><option value="5">หมู่ 5</option><option value="6">หมู่ 6</option><option value="7">หมู่ 7</option><option value="8">หมู่ 8</option><option value="9">หมู่ 9</option><option value="10">หมู่ 10</option><option value="11">หมู่ 11</option></select></div>
                      <div class="col-md-2"><select class="form-select form-select-sm" id="filterStatus" onchange="filterTable()"><option value="">ทั้งหมด</option><option value="poor">ครัวเรือนยากจน</option><option value="jp2t">จปฐ. ตกเกณฑ์</option></select></div>
                      <div class="col-md-2 text-end"><span class="text-xs text-muted" id="resultCount">—</span></div>
                  </div>
              </div>
          </div>
          ...
          <div class="card">
              <div class="card-body p-0"><div class="table-responsive"><table class="table table-hover mb-0">
                  <thead><tr><th class="ps-4">บ้านเลขที่</th><th>หมู่</th><th>เจ้าบ้าน</th><th class="text-center">สมาชิก</th><th class="text-center">ผู้สูงอายุ</th><th class="text-center">ผู้พิการ</th><th class="text-center">สถานะ</th><th class="pe-4 text-center">จัดการ</th></tr></thead>
                  <tbody id="householdBody"><tr><td colspan="8" class="text-center py-4 text-muted">กำลังโหลด...</td></tr></tbody>
              </table></div></div>
          </div>
      </div>
  </div>
  ...
  <!-- Add/Edit Modal -->
  <div class="modal fade" id="householdModal" tabindex="-1"><div class="modal-dialog modal-lg"><div class="modal-content border-0 shadow-lg" style="border-radius:16px;">
      <div class="modal-header border-0"><h5 class="modal-title fw-bold" id="modalTitle"><i class="fa-solid fa-house-chimney text-success me-2"></i>เพิ่มครัวเรือน</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
      <div class="modal-body">
          <form id="householdForm">
              <div class="row g-3">
                  <div class="col-md-6"><label class="form-label">รหัสประจำบ้าน <span class="text-danger">*</span></label><input type="text" class="form-control" id="fHouseID" required></div>
                  <div class="col-md-6"><label class="form-label">ชื่อเจ้าบ้าน <span class="text-danger">*</span></label><input type="text" class="form-control" id="fOwner" required></div>
                  
                  <div class="col-md-4"><label class="form-label">บ้านเลขที่ <span class="text-danger">*</span></label><input type="text" class="form-control" id="fHouseNo" required></div>
                  <div class="col-md-4"><label class="form-label">หมู่ที่ <span class="text-danger">*</span></label><input type="number" class="form-control" id="fVillageNo" min="1" max="20" required></div>
                  <div class="col-md-4"><label class="form-label">ชื่อหมู่บ้าน</label><input type="text" class="form-control" id="fVillageName"></div>
                  
                  <div class="col-md-12"><label class="form-label">สถานะ</label><select class="form-select" id="fPoverty"><option value="false">ปกติ</option><option value="true">ครัวเรือนยากจน</option></select></div>
  
                  <div class="col-12 mt-3">
                      <div class="d-flex justify-content-between align-items-center mb-2">
                          <label class="form-label fw-bold text-primary mb-0"><i class="fa-solid fa-map-location-dot me-1"></i>พิกัดแผนที่ (ลากหมุดเพื่อระบุตำแหน่ง)</label>
                          <button type="button" class="btn btn-sm btn-outline-primary" onclick="getCurrentLocation()"><i class="fa-solid fa-location-crosshairs me-1"></i>ดึงตำแหน่งปัจจุบัน</button>
                      </div>
                      <div id="pickerMap" style="height: 250px; border-radius: 8px; border: 1px solid #dee2e6;" class="mb-2"></div>
                      <div class="row g-2">
                           <div class="col-md-6"><div class="input-group input-group-sm"><span class="input-group-text">Lat</span><input type="text" class="form-control bg-light" id="fLat" readonly></div></div>
                           <div class="col-md-6"><div class="input-group input-group-sm"><span class="input-group-text">Lng</span><input type="text" class="form-control bg-light" id="fLng" readonly></div></div>
                      </div>
                  </div>
              </div>
          </form>
      </div>
      <div class="modal-footer border-0"><button type="button" class="btn btn-secondary" data-bs-dismiss="modal">ยกเลิก</button><button type="button" class="btn btn-success" onclick="saveHousehold()"><i class="fa-solid fa-save me-1"></i>บันทึก</button></div>
  </div></div></div>
  
  <!-- TR.14 Preview Modal -->
  <div class="modal fade" id="tr14PreviewModal" tabindex="-1">
      <div class="modal-dialog modal-xl modal-dialog-scrollable">
          <div class="modal-content">
              <div class="modal-header">
                  <h5 class="modal-title text-primary"><i class="fa-solid fa-file-invoice me-2"></i>ตรวจสอบข้อมูลก่อนนำเข้า ทร.14</h5>
                  <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
              </div>
              <div class="modal-body">
                  <div class="alert alert-info" id="tr14PreviewSummary">กำลังประมวลผลข้อมูล...</div>
                  <div class="table-responsive">
                      <table class="table table-bordered table-sm text-center align-middle" style="font-size: 0.9rem;">
                          ...
                          <tbody id="tr14PreviewBody">
                              <!-- Preview data goes here -->
                          </tbody>
                      </table>
                  </div>
              </div>
              <div class="modal-footer">
                  ...
                  <button type="button" class="btn btn-success" id="btnConfirmTR14Import" onclick="confirmTR14Import()"><i class="fa-solid fa-check me-2"></i>ยืนยันการนำเข้า</button>
              </div>
          </div>
      </div>
  </div>
...
> <script src="assets/js/digital-data-service.js"></script>
> <script src="assets/js/household-service.js"></script>
...
> <script>
  ...
  async function confirmTR14Import() {
      ...
      try {
          ...
          console.error(err);
          Swal.fire('เกิดข้อผิดพลาด', err.message || 'ไม่สามารถนำเข้าข้อมูลได้', 'error');
      }
  }
> </script>
  </body>
